...
interface HTNRegistryProps {
  onBack: () => void;
  currentRoom: string;
  onOpenRoomModal: () => void;
}

export const HTNRegistry: React.FC<HTNRegistryProps> = ({ onBack, currentRoom, onOpenRoomModal }) => {
  return (
    <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300 pb-10">
      ...
      <div className="bg-[#f8f9fa] border border-gray-200 rounded-sm px-4 py-1.5 flex items-center justify-between text-[12px]">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className="opacity-50">/</span>
          <span className="text-gray-400 font-medium">HTN Clinic</span>
        </div>
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-sm overflow-hidden">
        ...
        <div className="p-4 flex flex-col gap-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-6">
              <HtnFormField label="Duration of hypertension diagnosis" />
              <HtnFormField label="Family history of HTN, diabetes, stroke, heart disease" />
              <HtnFormField label="Past medical history" />
              <HtnFormField label="Allergies" />
              
              <HtnFormField label="Current medications (antihypertensives, other drugs)" />
              <HtnFormField label="Cardiovascular exam (heart sounds, edema, jugular venous pressure)" />
              <HtnFormField label="Fundoscopy (retinopathy signs)" />
              <HtnFormField label="Neurological exam (stroke risk signs)" />
              
              <HtnFormField label="Renal assessment (edema, urine output)" />
              <HtnFormField label="Smoking status" />
              <HtnFormField label="Alcohol intake" />
              <HtnFormField label="Physical activity level" />
              
              <HtnFormField label="Diet (salt intake, fat consumption)" />
              <HtnFormField label="Stress factors" />
              <HtnFormField label="Lifestyle modification advice (diet, exercise, stress management)" />
              <HtnFormField label="Counseling notes (adherence, danger signs)" />
              
              <div className="flex flex-col gap-1.5">
                <label className="text-[12px] font-bold text-gray-700">Next appointment date</label>
              ...
              <HtnFormField label="Referral to specialist (cardiologist, nephrologist, nutritionist)" />
              <HtnFormField label="Nurse's name" />
        ...
        {/* View: Hypertension Table Section */}
        <div className="border-t border-gray-200">
          <div className="px-4 py-2 bg-[#f8f9fa] border-b">
            <h2 className="text-[15px] font-medium text-gray-600">View: Hypertension</h2>
          </div>
               ...
               <table className="w-full text-left text-[13px]">
                 <thead className="bg-[#f8f9fa] text-gray-600 border-b">
                   <tr>
                     ...
                     <th className="px-4 py-2 font-bold text-[#333] border-r">Patient <i className="fa-solid fa-arrows-up-down text-[8px] opacity-30 ml-1"></i></th>
...
const HtnFormField: React.FC<{ label: string }> = ({ label }) => (
  <div className="flex flex-col gap-1.5">
