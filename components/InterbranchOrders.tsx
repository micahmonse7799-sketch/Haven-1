...
interface InterbranchOrdersProps {
  onBack: () => void;
  currentRoom: string;
  onOpenRoomModal: () => void;
}

export const InterbranchOrders: React.FC<InterbranchOrdersProps> = ({ onBack, currentRoom, onOpenRoomModal }) => {
  return (
    <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300 pb-20">
      ...
      <div className="bg-[#f8f9fa] border border-gray-200 rounded-sm px-4 py-1.5 flex items-center justify-between text-[12px]">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className="opacity-50">/</span>
          <span className="text-gray-400 font-medium">Interbranch Orders</span>
        </div>
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-4 grid grid-cols-1 lg:grid-cols-12 gap-6">
         ...
         <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4">
            <div className="flex flex-col gap-1.5">
               <label className="text-[12px] font-bold text-gray-700">Issuing Branch</label>
      ...
      {/* Interbranch Order Views Table */}
      <div className="bg-white border border-gray-200 shadow-sm rounded-sm overflow-hidden">
        <div className="px-4 py-2 border-b bg-[#f8f9fa]">
          <h2 className="text-[15px] font-medium text-gray-600">View: Interbranch Orders</h2>
        </div>
           ...
           <div className="border border-gray-200 rounded-sm overflow-x-auto min-h-[300px]">
             <table className="w-full text-left text-[12px] whitespace-nowrap">
               <thead className="bg-[#f8f9fa] text-gray-600 border-b">
                 <tr>
                   <th className="px-3 py-2 font-bold border-r">Order No <i className="fa-solid fa-arrows-up-down text-[8px] opacity-30 ml-1"></i></th>
                   ...
                   <th className="px-3 py-2 font-bold border-r">Issuing Branch <i className="fa-solid fa-arrows-up-down text-[8px] opacity-30 ml-1"></i></th>
               ...
               <tbody className="bg-white text-gray-400 italic">
                  <tr>
                    <td colSpan={8} className="text-center py-20">No data available in table</td>
                  </tr>
               </tbody>
             </table>
           </div>
